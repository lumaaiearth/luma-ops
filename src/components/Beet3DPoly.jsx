...
export default function Beet3DPoly({ placement, beetW, beetH, beetForm, L, shadow, cardBg }) {
  ...
  useEffect(() => {
    ...
    if (!t) return
    t.groundGroup.clear()
    const grid = new THREE.GridHelper(Math.max(beetW, beetH) + 8, Math.max(beetW, beetH) + 8,
      L ? 0xbcd3c2 : 0x24413156, L ? 0xdbe7dd : 0x1c332a)
    grid.material.opacity = L ? 0.5 : 0.35
    grid.material.transparent = true
    t.groundGroup.add(grid)
    let bedGeo
    if (beetForm === 'oval') {
      const shape = new THREE.Shape()
      shape.absellipse(0, 0, beetW / 2, beetH / 2, 0, Math.PI * 2)
      bedGeo = new THREE.ShapeGeometry(shape, 40)
    } else {
      bedGeo = new THREE.PlaneGeometry(beetW, beetH)
    }
    bedGeo.rotateX(-Math.PI / 2)
    const bed = new THREE.Mesh(bedGeo, new THREE.MeshLambertMaterial({ color: L ? 0xe6ecdd : 0x18241b }))
    bed.position.y = 0.005
    t.groundGroup.add(bed)
    camRef.current.dist = 0 // Auto-Distanz neu bestimmen
    sizeAndRender()
  }, [beetW, beetH, beetForm, L]) // eslint-disable-line
