...
- **Bewusste Abweichung von einer nicht normativen Empfehlung:**
  Die Quelle rät im Abschnitt „Example: Adding additional Kernel Types" — ausdrücklich mit dem Vorspann „_This section is non-normative._" — wörtlich: „Renderers that do not recognize the new kernel type should fall back to the default `ellipse` type."
  **BIOME folgt dem nicht.** Ein unbekannter Kernel wird abgewiesen, nicht als Ellipse gezeichnet. Begründung: der Kernel bestimmt laut derselben Quelle „an exact Gaussian type, i.e., its shape, attributes, intended rendering techniques"; eine Datei, die `customShape` sagt, als `ellipse` darzustellen hieße, eine andere Form zu zeigen als die, die in der Datei steht — und dem Betrachter nicht zu sagen, dass er etwas anderes sieht. Das ist derselbe Grundsatz, nach dem BIOME einen fehlenden Wert nicht als 0 zeigt. Die Empfehlung ist nicht normativ; die Abweichung steht hier, damit sie niemand für ein Versehen hält.
  Der **Rückfallweg der Basisspezifikation** über `COLOR_0` bleibt davon unberührt: er betrifft Renderer ohne Splat-Unterstützung, nicht die Kernelwahl.
- **Deckt ausdrücklich nicht:**
